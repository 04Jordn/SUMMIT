...
for _, leftover in ipairs({ "SummitUI", "SummitKeyUI", "SummitToasts" }) do
...
if isfolder and makefolder then
    for _, path in ipairs({ CONFIG.FOLDER, CONFIG.FOLDER .. "/configs" }) do
    ...
    for obj, props in Motors do
        for prop, m in props do
            ...
            for i = 1, #c do
                local comp = c[i]
                local g = comp[3]
                local offset, v0 = comp[1] - g, comp[2]
...
local function Tween(obj, props, time, damping, onDone)
    ...
    for prop, goal in props do
        ...
        if cur == goal then
            ...
            local gc, kind = Decompose(goal)
            local sc = Decompose(cur)
            if gc and sc and #gc == #sc then
                local prev, comps = reg[prop], table.create(#gc)
                for i = 1, #gc do
                    comps[i] = { sc[i], (prev and prev.c[i] and prev.c[i][2]) or 0, gc[i] }
                end
                if grp then grp.n += 1 end
                reg[prop] = { c = comps, f = freq, d = damp, k = kind, g = grp }
            else
                obj[prop] = goal
            end
        end
...
local function FlushIcons(sets)
    IconSets = sets
    for _, p in ipairs(PendingIcons) do pcall(ApplyIcon, p[1], p[2], p[3]) end
...
function Library:BuildTabs(windowObj) for _, t in ipairs(Private.Tabs) do t.Build(windowObj:MakeTab(t.Name, t.Subtitle, t.Icon), windowObj) end end
...
local function Bind(ctx, props, flag, getter, setter, extraFlags, loopEntry)
    ...
    local flags = { flag }
    if extraFlags then for _, f in ipairs(extraFlags) do flags[#flags + 1] = f end end
    -- idempotent, and separate from Destroy so DefineWidget can also run it off Destroying
    local torn = false
    function h.Teardown()
        if torn then return end
        torn = true
        for _, f in ipairs(flags) do UnregisterFlag(f) end
...
function Library:CreateWindow(titleText)
    ...
    function WindowObj:MakeTab(name, subtitle, icon)
        ...
        local function Select()
            ...
            selector.Size = UDim2.new(0, 3, 0, math.clamp(16 + dist * 0.35, 16, 42))
            Tween(selector, { Position = UDim2.new(0, 5, 0, selY) }, 0.26, 0.82)
            Tween(selector, { Size = UDim2.new(0, 3, 0, 16) }, 0.34)
...
function Library:Init()
    ...
    for i, t in ipairs(Private.Tabs) do t.Seq = i end
    ...
    self:RegisterTab("Settings", function(tab, windowObj)
        ...
        tab:AddButtons({
            { Name = "Server Hop", Callback = function()
                ...
                local open = {}
                for _, srv in ipairs(HttpService:JSONDecode(res).data) do
