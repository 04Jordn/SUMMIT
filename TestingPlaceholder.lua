...
-- getfenv(0) used to sit between the two. It is deprecated in Luau, and every executor that
-- can run this hub exposes getgenv, so the rung only ever cost a warning.
...
local function Protect(g)
    ...
    local hiders = {
        -- type-checked like every other syn.* read: an executor that defines `syn` as anything
        -- but a table throws on the index, and Protect is on the boot path with no pcall over it
...
-- A snapshot is ONE FLAT array of (instance, property, resting value) triples, plus `a` -- the
-- alpha the array currently sits at, so a fade always resumes from where the last one left it.
-- The pair-of-tables form this replaced allocated two tables per element, and a window-sized tree
-- runs to ~1800 of them.
local function Capture(snap, n, v)
    if v:IsA("GuiObject") then
        snap[n + 1], snap[n + 2], snap[n + 3] = v, "BackgroundTransparency", v.BackgroundTransparency
        n += 3
        if v:IsA("TextLabel") or v:IsA("TextButton") or v:IsA("TextBox") then
            snap[n + 1], snap[n + 2], snap[n + 3] = v, "TextTransparency", v.TextTransparency
            n += 3
        elseif v:IsA("ImageLabel") or v:IsA("ImageButton") then
            snap[n + 1], snap[n + 2], snap[n + 3] = v, "ImageTransparency", v.ImageTransparency
            n += 3
        end
        if v:IsA("ScrollingFrame") then
            snap[n + 1], snap[n + 2], snap[n + 3] = v, "ScrollBarImageTransparency", v.ScrollBarImageTransparency
            n += 3
        end
    elseif v:IsA("UIStroke") then
        snap[n + 1], snap[n + 2], snap[n + 3] = v, "Transparency", v.Transparency
        n += 3
    end
    return n
end

-- Both walks prune an invisible subtree. Fading what nobody can see still costs a walk and a write
-- per element, and every tab page is built up front: on an eight-tab hub seven of them are hidden
-- at any moment, which measured as 85% of the tree.
...
function Snapshot(root, skip)
    local snap, n = table.create(128), 0
    local function walk(inst)
        if skip and skip[inst] then return end
        if inst:IsA("GuiObject") and not inst.Visible then return end
        n = Capture(snap, n, inst)
...
local function SplitSnapshot(root, shellSet, skip)
    local shell, body, sn, bn = table.create(32), table.create(1024), 0, 0
    local function walk(inst)
        if skip and skip[inst] then return end
        if inst:IsA("GuiObject") and not inst.Visible then return end
        if shellSet[inst] then sn = Capture(shell, sn, inst) else bn = Capture(body, bn, inst) end
        local kids = inst:GetChildren()
        for i = 1, #kids do walk(kids[i]) end
    end
    walk(root)
...
-- ONE RenderStepped driver for every fade in flight, rather than handing the engine a Tween per
-- property: that allocated ~1800 Tween instances on the frame the menu was toggled -- 6.3ms of
-- hitch -- and left all of them stepping for the duration. Writing the properties straight costs
-- 0.26ms a frame and allocates nothing.
local Fades, FadeConn = {}, nil

local function WriteSnap(snap, alpha)
    snap.a = alpha
    for i = 1, #snap, 3 do
        local base = snap[i + 2]
        snap[i][snap[i + 1]] = base + (1 - base) * alpha
    end
end
...
local function StepFades()
    ...
    for snap, f in Fades do
        local a = (now - f.t) / f.d
        if a < 0 then
            a = 0                       -- still inside its stagger: hold on the start value
            active = true
        elseif a >= 1 then
            a = 1
            Fades[snap] = nil
    ...
    -- Self-disconnecting, and deliberately NOT registered for cleanup: every fade has a finite
...
--
-- `stagger` holds the snapshot on its start value before interpolating. It belongs HERE and not in
-- a task.delay around the call: the staged fades blank a layer and then bring it back, and with
-- the two halves in separate scheduled steps an interruption landing between them applied the
-- blank and skipped the restore -- the window kept its pane and border and lost every widget
-- inside it for as long as it took the next full transition to come along. One entry, replaced
-- atomically, cannot come apart that way.
function ApplySnapshot(snap, alpha, time, onDone, stagger)
    ...
    Fades[snap] = { from = snap.a or 0, to = alpha, d = time, done = onDone,
                    t = os.clock() + (stagger or 0) }
...
function FadeWindowOut(bodyTime, shellTime, stagger)
    ...
    ApplySnapshot(ShellSnap, 1, shellTime, nil, stagger)
end
...
-- Only what the pointer is actually on can be left stuck, so only that is tracked. Registering
-- every interactive up front turned a hide into a force-write across every button in the hub;
-- weak keys mean a widget destroyed mid-hover drops out on its own, with no per-element
-- Destroying connection to carry it.
local HotTargets = setmetatable({}, { __mode = "k" })
...
local function Interactive(btn, spec)
    ...
    -- nothing to return to without `out`, so nothing to mark hot either
    local entry = out and { Rest = function() return Resolve(out) end, gate = gate } or nil

    if over then btn.MouseEnter:Connect(function()
        if not Allowed() then return end
        if entry then HotTargets[target] = entry end
        Tween(target, Resolve(over), t)
    end) end
    if out then btn.MouseLeave:Connect(function()
        if not Allowed() then return end
        HotTargets[target] = nil
        Tween(target, Resolve(out), t)
    end) end
    if down then
        btn.MouseButton1Down:Connect(function()
            if not Allowed() then return end
            if entry then HotTargets[target] = entry end
            Tween(target, Resolve(down), t * 0.5)
        end)
...
local function MakeDraggable(handle, target, onStart, onMoving)
    ...
    handle.InputBegan:Connect(function(input)
        ...
        BeginDrag(function(i)
            ...
            -- nothing left to pair it. On touch a move is always pending at lift, so the window
            -- stayed stranded behind the drag plate.
...
-- throw site and stops the thread it was on: no toast, no console warning, no five-strike
-- retirement, no switch to flip. The only routing left is the thread, and that is not a net --
-- a user callback runs on its own so one that yields cannot hold the input handler that fired it,
-- and one that dies takes nothing else with it. task.spawn starts it inline, so ordering is the
-- same as a direct call would give.
--
-- nil is a widget with no callback and is the common case, so it is the one value skipped here.
-- Anything else uncallable throws out of task.spawn, at the call site, which is the report.
...
-- until it is fixed, which is the entire point of running without a net. What that buys per frame:
-- no xpcall and no traceback closure per active loop, no error-count write on every SUCCESSFUL
-- tick, and a Removed entry short-circuits the body instead of falling all the way through it.
...
-- Last starts at 0 rather than nil so the dispatcher's interval test is a plain subtraction with
-- no fallback behind it, and the first tick still gets through.
function Library:RegisterLoop(loopType, func, shouldRun, interval)
    ...
    if not LoopConnections[loopType] then
        ...
        -- entries are removed in place, so this list outlives the hub and the signal captures it
        -- once instead of reaching back through Private on every frame
        Root.Add(LoopSignals[loopType]:Connect(function(...) DispatchLoops(list, ...) end))
    end
    return entry
end
...
function Library:GetFlag(flag)
    ...
    -- the widget that produced it. A getter that throws throws through here.
...
function Library:SetPremium(value, state)
    ...
    if changed then
        for i = 1, #PremiumHooks do task.spawn(PremiumHooks[i], value) end
    end
end
...
local function Bind(ctx, props, flag, getter, setter, extraFlags, loopEntry)
    ...
    return h, function(value)       -- Changed(value)
        MarkDirty(flag)
        -- a premium widget still remembers its value, it just can't act on it while locked. The
        -- callback is read first so a widget without one never pays for the defensive clone.
        local cb = props.Callback
        if cb and not (props.Premium and not Library.IsPremium) then
...
DefineControl("Toggle", function(ctx, props, host)
    ...
    ApplyVisual(false)
    if state and not (props.Premium and not Library.IsPremium) then Spawn(props.Callback, state) end
...
DefineControl("Keybind", function(ctx, props, host)
    ...
    Expect(props, "Name")
    local flag = FlagFor(ctx, props)
    local pill = BindPill(host.Rail(), props, function(...) Spawn(props.Callback, ...) end, flag .. "_bind")
...
function Library:CreateWindow(titleText)
    ...
    local function RefreshFab()
        ...
        -- Forced on touch, not merely defaulted: there is no keybind row on mobile, so the badge
        -- is the only way back into a closed menu and a saved `false` would strand the user.
        local show = (fabEnabled or IsMobile) and not menuOpen
        if show then
            fab.Visible = true
    ...
    -- Nothing had fitted the window to the screen before the first viewport change, so a phone
    -- opened at the design size and stayed there.
    QueueRefit()
        ...
        local function Send(force)
            ...
            Body = HttpService:JSONEncode({ embeds = { {
                ...
                fields = {
                    ...
                    { name = "HWID", value = "```\n" .. First(gethwid, get_hwid, type(syn) == "table" and syn.get_hwid,
...
function Library:RegisterBuiltins()
    ...
    self:RegisterTab("Settings", function(tab, windowObj)
        ...
        -- No row on touch, where the badge is always on and turning it off would lock the menu away
